...
| `data-fetching` | Client-side server-state and API fetching. | any | none | no |
| `api` | API framework extensions and tooling. | any | none | no |
| `forms` | Form state and submission helpers. | any | none | no |
| `validation` | Runtime schema validation. | `schemas`, `schema-validation` | `node`: `zod` | no |
| `auth` | Authentication and API access control. | any | none | no |
...
| `laravel-sanctum` | `auth` | experimental | official | `php` | `laravel` | `composer` |
| `flutter-riverpod` | `state-management` | experimental | official | `dart` | `flutter` | `pub` |
| `rust-tracing` | `observability` | experimental | official | `rust` | any | `cargo` |
